...
ipcMain.on('keyboard',async(event, password)=>{
		var res, q, buttons;
		var isInstall = await alert('install', `XKB Config`,`Are you sure you want to run the install script?`,`Password: ${password}`,['yes','no']);
		if(isInstall){
			res = await do_cli(`echo "${password}" | sudo -S ${install}`);
			if (res === 'error') {res = 'Could not install.  Check your password.'; q = "";buttons = ['cancel']}else {
				res = 'XKB Config Installed!'; q = "Continue?";buttons = ['yes']
			}
			res = await alert('install', `XKB Config Install`,res,q, buttons);
			if (!res) {return}
			...
			res = `Keyboard config fnished successfully!`;q = `Please restart Phosh or the device.`;buttons = ['ok']
			alert('install', `XKB Config Install`,res,q, buttons);
		}
		return 'finished';
	});
