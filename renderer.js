...
try{
  ...
  if(distro === 'Manjaro-ARM'){document.getElementById('kb_config_app_name').innerHTML = 'xkeyboard-config'}
  ...
  json.forEach((item)=>{
	  ...
	  if(item.command.includes('git clone')||item.command.includes('git push')||item.command.includes('git pull')){
		...
		<button class="delete" id="${item.name}">Delete Command</button>`
	}else{buttons = `<button class="copy" id="${item.name}" data-command='${clone_command}'>Copy</button><button class="delete" id="${item.name}">Delete Command</button>`;}
